...
- (void)startPrefetchingAtIndex:(NSUInteger)index
{
    ...
    [self.manager downloadWithURL:self.prefetchURLs[index] options:self.options progress:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished)
    {
        ...
        if (image)
        {
            ABLoggerDebug(@"Prefetched %d out of %d", self.finishedCount, self.prefetchURLs.count);
        }
        else
        {
            ABLoggerDebug(@"Prefetched %d out of %d (Failed)", self.finishedCount, [self.prefetchURLs count]);
...
- (void)reportStatus
{
    ...
    ABLoggerDebug(@"Finished prefetching (%d successful, %d skipped, timeElasped %.2f)", total - self.skippedCount, self.skippedCount, CFAbsoluteTimeGetCurrent() - self.startedTime);
}
