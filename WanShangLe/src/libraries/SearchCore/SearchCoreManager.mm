...
- (BOOL)GetPinYin:(NSNumber*)localID pinYin:(NSMutableString*)pinyinDes matchPos:(NSMutableArray*)matchPosInPinYin matchCNPos:(NSRange *)matchPosInPChinese{
    [pinyinDes setString:@""];

    u2char pinyinBuf[256];
    
    NSMutableArray *posInCh = [[NSMutableArray alloc] initWithCapacity:10];
    ...
    //chinese position
    Array* aMatchPosInChinese = NULL;
	if (matchPosInPChinese ) {
		aMatchPosInChinese = new Array;
		ArrayInit(aMatchPosInChinese);
    }
	
	BOOL result = Tree_GetPinYin(&iSearchTree,[localID intValue],pinyinBuf,aMatchPosInPinYin,aMatchPosInChinese);
    ...
    if (matchPosInPChinese) {
        [self ArrayToNSArray:aMatchPosInChinese NSArray:posInCh];
        aMatchPosInChinese->Reset(aMatchPosInChinese);
		delete aMatchPosInChinese;
        
        matchPosInPChinese->location = [[posInCh objectAtIndex:0] intValue];
        
        if (matchPosInPChinese->location == 0) {
             matchPosInPChinese->length = [[posInCh lastObject] intValue]+1;
        }else{
